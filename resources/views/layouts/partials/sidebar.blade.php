...
<aside class="fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-slate-200/80 shadow-sidebar transform transition-transform duration-200 lg:translate-x-0"
    ...
    <div class="flex flex-col h-full">
        <div class="flex items-center gap-3 px-6 py-5 border-b border-slate-100">
            <div class="w-10 h-10 rounded-xl bg-gradient-to-br {{ $isPlatformPanel ? 'from-slate-700 to-slate-900' : ($isStudentPanel || $role === 'learner' ? 'from-emerald-500 to-teal-600' : 'from-indigo-500 to-violet-600') }} flex items-center justify-center shadow-lg {{ $isPlatformPanel ? 'shadow-slate-500/30' : ($isStudentPanel || $role === 'learner' ? 'shadow-emerald-500/30' : 'shadow-indigo-500/30') }}">
