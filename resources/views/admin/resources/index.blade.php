...
<div class="space-y-6">
    ...
    <p class="text-sm text-slate-500 -mt-2">Reach more potential learners with free resources.</p>
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2 shrink-0">
            ...
            <a href="{{ route('admin.resources.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800 transition">
    ...
    <div class="glass-card rounded-2xl overflow-hidden">
        <div class="px-4 sm:px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <form method="GET" class="flex flex-wrap items-center gap-2">
                ...
                @if(request('sort'))
                    <input type="hidden" name="sort" value="{{ request('sort') }}">
                @endif
                <span class="px-3 py-2 rounded-lg border border-slate-200 bg-slate-50 text-xs font-medium text-slate-600 inline-flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"/></svg>
                    Filters
                </span>
                <select name="status" onchange="this.form.submit()" class="px-3 py-2 rounded-lg bg-white border border-slate-200 text-sm text-slate-700 focus:outline-none">
            ...
            <table class="w-full text-sm panel-table">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-indigo-50/40">
                        <td class="px-6 py-4 font-semibold text-slate-800">{{ $resource->title }}</td>
                        <td class="px-6 py-4">
                            <x-badge type="info">{{ strtoupper($resource->resource_type) }}</x-badge>
                        </td>
                        <td class="px-6 py-4">
                            <x-badge :type="match($resource->status) { 'published' => 'success', 'draft' => 'warning', default => 'default' }">
                                {{ ucfirst($resource->status) }}
                            </x-badge>
                        </td>
                        <td class="px-6 py-4 text-slate-500">{{ $resource->created_at->format('M d, Y') }}</td>
                        <td class="px-6 py-4 text-right">
                            <a href="{{ route('admin.resources.edit', $resource) }}" class="text-indigo-600 hover:text-indigo-800 text-sm mr-3">Edit</a>
                            <form method="POST" action="{{ route('admin.resources.destroy', $resource) }}" class="inline">
                                @csrf
                                @method('DELETE')
        ...
        <div class="px-6 py-4 border-t border-slate-200">{{ $resources->links() }}</div>
        @else
        <div class="py-20 px-6 text-center">
            ...
            <p class="text-sm text-slate-500 mt-2">Add free PDFs, audio, videos, and more for learner access.</p>
            <a href="{{ route('admin.resources.create') }}" class="inline-flex items-center gap-2 mt-6 px-5 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800 transition">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                Create
            </a>
        </div>
