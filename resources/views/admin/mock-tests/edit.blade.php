...
@section('page-title', 'Edit Mock Test')
@section('breadcrumb', 'Mock Tests / Edit')

@section('content')
<div class="max-w-3xl space-y-6" x-data="{
    title: @js(old('title', $mockTest->title)),
    isFree: @js((bool) old('is_free', $mockTest->is_free)),
    quizType: @js(old('quiz_type', $mockTest->quiz_type))
}">
    <a href="{{ route('admin.mock-tests.index') }}" class="inline-flex items-center gap-1 text-sm text-indigo-600 hover:text-indigo-800 font-medium">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        Back
    </a>

    <p class="text-sm text-slate-500 -mt-2">Update mock test details.</p>

    <div class="glass-card rounded-2xl p-6 sm:p-8">
        <form method="POST" action="{{ route('admin.mock-tests.update', $mockTest) }}" id="mock-test-edit-form" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-3">
                ...
                <div class="relative">
                    ...
                    <input type="number"
                           ...
                           min="0"
                           placeholder="Price"
                           value="{{ old('price', $mockTest->price) }}"
                           :disabled="isFree"
                ...
                <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox"
                           ...
                           value="1"
                           x-model="isFree"
                           @checked(old('is_free', $mockTest->is_free))
            ...
            <div class="space-y-1.5">
                ...
                <select name="template" id="template" class="panel-select w-full">
                    ...
                    @foreach($templates as $key => $label)
                        <option value="{{ $key }}" @selected(old('template', $mockTest->template) === $key)>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="space-y-1.5">
                ...
                <select name="status" id="status" class="panel-select w-full">
                    @foreach(['draft', 'published', 'unpublished'] as $status)
                        <option value="{{ $status }}" @selected(old('status', $mockTest->status) === $status)>{{ ucfirst($status) }}</option>
                    @endforeach
                </select>
    ...
    <div class="flex items-center gap-3">
        <button type="submit" form="mock-test-edit-form" class="px-6 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800 transition shadow-sm">
            Save Changes
        </button>
