...
<div class="space-y-6">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2 shrink-0">
            ...
            <a href="{{ route('admin.ebooks.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800 transition">
    ...
    <div class="glass-card rounded-2xl overflow-hidden">
            ...
            <table class="w-full text-sm panel-table">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-indigo-50/40">
                        <td class="px-6 py-4 font-semibold text-slate-800">{{ $ebook->title }}</td>
                        <td class="px-6 py-4">
                            @if($ebook->is_free)
                                <span class="text-emerald-600 font-medium">Free</span>
                            @else
                                ₹{{ number_format($ebook->price, 0) }}
                            @endif
                        </td>
                        <td class="px-6 py-4">{{ $ebook->contentSecurityLabel() }}</td>
                        <td class="px-6 py-4">
                            <x-badge :type="match($ebook->status) { 'published' => 'success', 'draft' => 'warning', default => 'default' }">
                                {{ ucfirst($ebook->status) }}
                            </x-badge>
                        </td>
                        <td class="px-6 py-4 text-slate-500">{{ $ebook->created_at->format('M d, Y') }}</td>
                        <td class="px-6 py-4 text-right">
                            <form method="POST" action="{{ route('admin.ebooks.destroy', $ebook) }}" class="inline">
                                @csrf
                                @method('DELETE')
        ...
        <div class="px-6 py-4 border-t border-slate-200">{{ $ebooks->links() }}</div>
        @else
        <div class="py-20 px-6 text-center">
            ...
            <p class="text-sm text-slate-500 mt-2">Create your first ebook to get started.</p>
            <a href="{{ route('admin.ebooks.create') }}" class="inline-flex items-center gap-2 mt-6 px-5 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800 transition">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                Create
            </a>
        </div>
