...
@section('breadcrumb', 'Ebooks / Create Ebook')
...
<div class="max-w-3xl space-y-6" x-data="{
    ...
    <a href="{{ route('admin.ebooks.index') }}" class="inline-flex items-center gap-1 text-sm text-indigo-600 hover:text-indigo-800 font-medium">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        Back
    </a>

    <p class="text-sm text-slate-500 -mt-2">Start creating a new ebook.</p>

    <div class="glass-card rounded-2xl p-6 sm:p-8">
        <form method="POST" action="{{ route('admin.ebooks.store') }}" id="ebook-create-form" class="space-y-6">
            @csrf
            ...
            <div class="space-y-3">
                ...
                <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                    ...
                    <span class="text-sm text-slate-600">Make this a free ebook</span>
                </label>
            </div>
    ...
    <div class="flex items-center gap-3">
        <button type="submit" form="ebook-create-form" class="px-6 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-800 transition shadow-sm">
            Create
        </button>
        <a href="{{ route('admin.ebooks.index') }}" class="px-6 py-2.5 rounded-xl bg-slate-100 text-slate-700 text-sm font-semibold hover:bg-slate-200 transition">
            Cancel
        </a>
    </div>
</div>
