@php
    $pricing = $settings->android_pricing ?? [];
@endphp
...
<div class="mt-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
    Android in-app purchases may require a platform add-on and Google Play Console product setup before learners can buy.
</div>
...
<form method="POST" action="{{ route('admin.courses.settings.update', [$course, $panel]) }}" class="mt-8 space-y-6" @change="markDirty()" x-on:input="markDirty()">
    ...
    <div class="flex items-center justify-between rounded-xl border border-slate-200 px-4 py-3">
        <div>
            <p class="text-sm font-medium text-slate-800">Enable Android pricing</p>
            <p class="text-xs text-slate-500">Offer this course via Google Play billing</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1.5">Play product ID</label>
