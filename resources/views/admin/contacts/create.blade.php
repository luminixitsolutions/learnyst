...
<div class="max-w-3xl space-y-6">
    ...
    <div class="glass-card rounded-2xl p-6 sm:p-8">
        <form method="POST" action="{{ route('admin.contacts.store') }}" class="space-y-5">
            @csrf
            ...
            <x-form-input label="Organization" name="organization" :value="old('organization')" />
            <x-form-input label="Contact Type" name="contact_type" type="select" required>
                @foreach(['lead' => 'Lead', 'customer' => 'Customer', 'partner' => 'Partner', 'vendor' => 'Vendor'] as $value => $label)
                    <option value="{{ $value }}" @selected(old('contact_type', 'lead') === $value)>{{ $label }}</option>
                @endforeach
            </x-form-input>
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="panel-btn-primary">Create Contact</button>
