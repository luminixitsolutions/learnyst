...
<div class="space-y-8">
    <div>
        <h3 class="text-2xl font-bold text-slate-900">Classification</h3>
    </div>
        ...
        @foreach($options as $option)
            <a href="{{ $option['url'] }}"
               class="group glass-card rounded-2xl p-6 flex flex-col gap-4 hover:border-emerald-200 hover:shadow-md transition-all duration-200">
                ...
                <div>
                    <h4 class="text-base font-bold text-slate-900 group-hover:text-emerald-700 transition-colors">{{ $option['title'] }}</h4>
                    <p class="text-sm text-slate-500 mt-2 leading-relaxed">{{ $option['description'] }}</p>
                </div>
            </a>
        @endforeach
    </div>
