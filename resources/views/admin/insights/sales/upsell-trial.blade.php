...
<div class="space-y-6">
    ...
    <div class="glass-card rounded-2xl overflow-hidden panel-datatable-wrapper">
        ...
        <div class="overflow-x-auto">
            <table id="upsellTrialTable" class="w-full text-sm panel-table display" style="width:100%">
                <thead>
                    <tr class="text-left">
                        <th class="px-6 py-4">Learner</th>
                        <th class="px-6 py-4">Email</th>
                        <th class="px-6 py-4">Trial Product</th>
                        <th class="px-6 py-4">Purchased Product</th>
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-indigo-50/40">
                        ...
                        <td class="px-6 py-4 text-slate-600">{{ $row->user?->orders()->where('payment_status','paid')->latest()->first()?->order_number ?? '—' }}</td>
...
@push('scripts')
@if($records->count())
    <x-admin.datatable-scripts table-id="upsellTrialTable" entity="upsell users" :order-column="0" order-direction="desc" export-file-name="upsell-trial" />
@endif
@endpush
