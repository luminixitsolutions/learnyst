...
@section('breadcrumb', 'Platform Admin / Solution Pages / ' . $meta['label'])
...
<div class="space-y-6 max-w-4xl">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <a href="{{ route('platform.solution-pages.index') }}" class="text-sm text-indigo-600 hover:underline">← All Solution Pages</a>
            <p class="text-sm text-slate-500 mt-1">{{ $meta['description'] }}</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('website.solution', $slug) }}" target="_blank" class="panel-btn-secondary">Preview page</a>
            <form method="POST" action="{{ route('platform.solution-pages.reset', $slug) }}" onsubmit="return confirm('Reset this solution page to defaults?')">
                @csrf
                @method('DELETE')
    ...
    <form method="POST" action="{{ route('platform.solution-pages.update', $slug) }}" enctype="multipart/form-data" class="glass-card rounded-2xl p-6 space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <x-form-input label="Caption" name="caption" :value="old('caption', $content['caption'] ?? 'Solutions')" />
        ...
        <div class="flex items-center gap-3 pt-2 border-t border-slate-100">
            ...
            <a href="{{ route('platform.solution-pages.index') }}" class="text-sm text-slate-500 hover:text-slate-700">Cancel</a>
        </div>
    </form>
</div>
